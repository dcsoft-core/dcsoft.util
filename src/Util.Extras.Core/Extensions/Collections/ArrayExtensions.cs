    ...
    public static partial class ArrayExtensions
    {
        ...
        public static T[] CombineArray<T>(this T[] combineWith, T[] arrayToCombine)
        {
            if (combineWith != default(T[]) && arrayToCombine != default(T[]))
            {
                ...
                System.Array.Resize(ref combineWith, initialSize + arrayToCombine.Length);
                System.Array.Copy(arrayToCombine, arrayToCombine.GetLowerBound(0), combineWith, initialSize,
        ...
        public static T[] BlockCopy<T>(this T[] source, int index, int length) =>
            BlockCopy(source, index, length, false);
        ...
        public static T[] BlockCopy<T>(this T[] source, int index, int length, bool padToLength)
        {
            ...
            if (b == null)
                b = new T[n];
            System.Array.Copy(source, index, b, 0, n); // 从source数组指定索引开始复制数据到b数组当中，直至到达指定长度结束复制
            return b;
        }
