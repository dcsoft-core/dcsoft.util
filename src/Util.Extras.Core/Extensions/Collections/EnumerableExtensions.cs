    ...
    public static class EnumerableExtensions
    {
        ...
        public static bool EqualsTo<T>(this IEnumerable<T> sourceList, IEnumerable<T> targetList)
        {
            ...
            if (!sourceList.Except(targetList).Any() && !targetList.Except(sourceList).Any())
                return true;
            // 深度对比
            var sourceListStr = Json.ToJson(sourceList).Trim();
            var targetListStr = Json.ToJson(targetList).Trim();
        ...
        /// <param name="condition">第三方条件</param>
        /// <exception cref="ArgumentNullException"></exception>
        public static IEnumerable<T> WhereIf<T>(this IEnumerable<T> enumerable, Func<T, int, bool> predicate,
            bool condition)
        {
            if (enumerable == null)
                throw new ArgumentNullException(nameof(enumerable));
