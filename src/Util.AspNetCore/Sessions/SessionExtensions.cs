...
using Util.Helpers;
using ClaimTypes = Util.Sessions.Claims.ClaimTypes;
...
namespace Util.Sessions
{
    ...
    public static class Extensions
    {
        ...
        {
            var result = Web.Identity.GetValue(ClaimTypes.UserName);
            return string.IsNullOrWhiteSpace(result)
                ? Web.Identity.GetValue(System.Security.Claims.ClaimTypes.Name)
                : result;
        }
        ...
        /// </summary>
        /// <param name="session">用户会话</param>
        public static string GetNickName(this ISession session)
        {
            return Web.Identity.GetValue(ClaimTypes.NickName);
        }
        ...
        {
            var result = Web.Identity.GetValue(ClaimTypes.Email);
            return string.IsNullOrWhiteSpace(result)
                ? Web.Identity.GetValue(System.Security.Claims.ClaimTypes.Email)
                : result;
        }
        ...
        {
            var result = Web.Identity.GetValue(ClaimTypes.Mobile);
            return string.IsNullOrWhiteSpace(result)
                ? Web.Identity.GetValue(System.Security.Claims.ClaimTypes.MobilePhone)
                : result;
        }
        ...
        {
            return Web.Identity.GetValue(ClaimTypes.UserType).ToInt();
        }
